...
#definition[Partial order][
  A *partial order* on a set $P$ is a binary relation $<= subset.eq P times P$
...
#definition[Chain][
  A *chain* in a poset $(P, <=)$ is a subset $C subset.eq P$
  such that any two elements of $C$ are comparable.
]

#definition[Maximal element][
  An element $m in P$ is *maximal* if there is no $x in P$ with $m < x$.
]

#lemma[Zorn's][
  Let $(P, <=)$ be a partially ordered set. If every chain in $P$
  has an upper bound in $P$, then $P$ contains a maximal element.
]

#axiom[Of choice][
  Let $X$ be a nonempty set and
  let $cal(P)(X)$ be the set of all nonempty subsets of $X$.
  Then there exists a map $f : cal(P)(X) -> X$ such that
  $ f(A) in A quad forall A in cal(P)(X). $
]
...
#remark[
  ...
  - to omit a requirement for a multiplicative identity: \
    "pseudo#sym.hyph.nobreak;ring" or
    "rng" ("ring" without "i"; pronounced _rung_),
    although the former also has other meanings.
]
...
#definition[Commutative rings][
  The following are each proper subsets as in order:

  / Ring:
    is an rng with a _multiplicative identity_ 1.
  
  / Commutative ring:
    is a ring with _commutative multiplication_
    (thus forming an abelian group).

  / Integral domain:
    is a nonzero commutative ring
    in which the _product of any two nonzero elements is nonzero_
    or, equivalently, in which there are _no nonzero zero divisors_.
  ...
  / GCD domain:
    ...
    every pair of _nonzero elements has a greatest common divisor_.

  / Unique Factorization Domain (UFD):
    is an integral domain where every _nonzero nonunit element
    can be written as a product of irreducible elements_,
  ...
  / Euclidean Domain:
    ...
    for a form of the _division algorithm_.
  
  / Field:
    is a commutative division ring
    with a nontrivial multiplicative identity $1 != 0$.
  
  / Algebraically closed field:
    is a field in which every nonconstant polynomial has a root.

  #divider()

  / Noetherian ring:
    is a commutative ring with $1$ such that every ideal
    of the ring is finitely generated.
  
  / Dedekind domain:
    is an integral domain in which every nonzero ideal
...
#definition[Reducible polynomial][
  A polynomial is *reducible* if it can be factored into a product of
...
#lemma[Gauss's][
  ...
  The product of two primitive polynomials in $R[x]$ is also primitive.
...
#theorem[
  ...
  Inheritance for the polynomial ring $R[x]$ over a ring $R$:
...
#theorem[
  ...
  $f(x)$ has a root in $F$.
  If $alpha$ is the root then the factor is $(x - a)$.
]
...
#definition[Field][
  ...
  + $(F\\{0}, dot)$ is an abelian group with identity $1 != 0$.
  + Multiplication distributes over addition:\
...
#examples[
  $QQ, RR, CC, RR(x), QQ(i),$
  and $FF_(p^k)$ for prime $p$ and positive integer $k$.
]

#theorem[
  There are no fields of order 1.
]

#proof[
  The only ring (up to isomorphism) of order 1 is the trivial ring,
  which is not a field since its multiplicative identity is trivial.
]
...
#corollary[
  If $F$ is a finite field, then its characteristic is a prime number.
]

#definition[Prime subfield][
  The *prime subfield* of a field $F$ is the smallest subfield of $F$.
  It is isomorphic to either $QQ$ or $FF_p$ for some prime $p$.
]
...
#definition[Field extension][
  If $F$ is a subfield of a field $K$,
  then $K$ is an *extension~(field)* of the *base~(field)* $F$.
  This relation is denoted $K / F$ or $K : F$.
]

#definition[Generated field][
  Let $S subset.eq K$ be a subset of a field extension $K slash F$.
  The *field generated by $S$ over $F$* is
  the smallest subfield of $K$ containing both $F$ and $S$,
  denoted $F(S)$ or $F(a, beta, ...)$ if $S = {alpha, beta, ...}$.
]

#definition[Simple extension][
  A field $K$ is *simple extension* of a field $F$ if $K = F(alpha)$
  for some $alpha in K$ which is then said to be a *primitive element*.
]

#theorem[
  Every finite extension of a field of characteristic 0
  is a simple extension.
]

#definition[Degree][
  The *degree* of a field extension $K slash F$
  is the dimension of $K$ as a vector space over $F$,
  denoted $[K : F] = dim_F K$.

  The cardinality of an extension is that of its degree.
]

#theorem[
  Let $F subset.eq K subset.eq L$ be fields. Then
  $ [L : F] = [L : K] [K : F]. $
]
...
let $alpha in K$ be an element,
and let $p(x) in F[x]$ be an irreducible polynomial.

#theorem[
  $ exists alpha in K : p(alpha) = 0
  quad ==> quad
  F(alpha) tilde.equiv F[x] slash (p(x)) $
]

#definition[Algebraic element][
  If there exists a nonzero polynomial $p(x) in F[x]$ such that $p(alpha) = 0$,
  then $alpha$ is *algebraic over $F$*.
  Otherwise, $alpha$ is *transcendental over $F$*.
]

#theorem[
  If $K$ is finite, then every element of $K$ is algebraic over $F$.
]

Let $alpha in K$ be algebraic over $F$.

#definition[Minimal polynomial][
  The *minimal polynomial of $alpha$ over $F$* is the unique monic polynomial
  $p(x) in F[x]$ of least degree such that $p(alpha) = 0$.
  Its degree is called the *degree of $alpha$*.
]

Let $p(x) in F[x]$ be the minimal polynomial of $alpha$ over $F$,

#theorem[
  $p(x)$ exists and it is such that
  $ q(alpha) = 0 quad <==> quad p(x) | q(x) $
  for every polynomial $q(x) in F[x]$.
]
...
#definition[Composite field][
  The *composite field* of two subfields $F_1$ and $F_2$ of a field $K$
  is the smallest subfield of $K$ containing both $F_1$ and $F_2$,
  denoted $F_1 F_2$.
]

#image("assets/image-15.png")

#image("assets/image-16.png", width: 15em)
...
#theorem[
  If $V$ is an $n$-dimensional vector space over $F$,
  then $V tilde.equiv F^n$.
]

#theorem[
  If $V$ is finite, then $V tilde.equiv V^*$.
  Otherwise, $dim V < dim V^*$. 
]

#theorem[
  The matrix of a dual linear map
  with respect to dual bases is
  the transpose of the matrix of the original linear map
  with respect to the original bases.
]

#corollary[
  The row rank equals the column rank for any matrix.
]
